...
const lobbies = new Map(); // textChannel.id -> lobbyData
...
client.on("messageCreate", async (msg) => {
  ...
  if (msg.content === "!lobbies") {
    if (lobbies.size === 0) return msg.channel.send("Сейчас нет активных лобби.");
    let text = "**Активные лобби:**\n";
    for (const [id, lobby] of lobbies) {
      const user = await client.users.fetch(lobby.owner).catch(() => null);
      text += `- ${user ? user.tag : "неизвестный пользователь"} (${lobby.limit} игроков)\n`;
    }
  ...
  if (msg.content.startsWith("!kick")) {
    ...
    if (!lobby || msg.author.id !== lobby.owner)
  ...
  if (msg.content === "!close") {
    ...
    if (msg.author.id !== lobby.owner) return msg.reply("Только создатель может закрыть лобби.");
...
client.on("messageReactionAdd", async (reaction, user) => {
  ...
  const text = await message.guild.channels.create({
    name: `${game.toLowerCase().replace(" ", "-")}-${user.username}`,
    type: 0,
    parent: category ?? undefined,
    permissionOverwrites: [
      {
