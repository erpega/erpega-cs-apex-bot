const { Client, GatewayIntentBits, Partials, PermissionsBitField } = require("discord.js"); const express = require("express"); require("dotenv").config();

const app = express(); app.get("/", (req, res) => res.send("Bot is alive!")); app.listen(3000, () => console.log("Web server started"));
...
if (msg.content === "!help") { return msg.channel.send( "Команды:\n" + "!kick @пользователь — кикнуть игрока из голосового канала (только для создателя)\n" + "!close — закрыть лобби и удалить каналы (только для создателя)\n" + "!lobbies — показать список активных лобби" ); }

if (msg.content === "!lobbies") { if (lobbies.size === 0) return msg.channel.send("Сейчас нет активных лобби."); let text = "Активные лобби:\n"; for (const [id, lobby] of lobbies) { const user = await client.users.fetch(lobby.owner).catch(() => null); text += - ${user ? user.tag : "неизвестный пользователь"} (${lobby.limit} игроков)\n; } return msg.channel.send(text); }

if (msg.content.startsWith("!kick")) { const lobby = lobbies.get(msg.channel.id); if (!lobby || msg.author.id !== lobby.owner) return msg.reply("Ты не создатель этого лобби.");
...
client.on("messageReactionAdd", async (reaction, user) => { if (user.bot) return; const { message } = reaction; if (!message.guild || !["🔫", "⚔️", "🔮"].includes(reaction.emoji.name)) return;
...
if (reaction.emoji.name === "🔫") { game = "CS 2"; limit = 5; } else if (reaction.emoji.name === "⚔️") { game = "Apex Legends"; limit = 3; } else if (reaction.emoji.name === "🔮") { game = "Dota 2"; limit = 5; }
...
const text = await message.guild.channels.create({ name: ${game.toLowerCase().replace(" ", "-")}-${user.username}, type: 0, parent: category ?? undefined, permissionOverwrites: [ { id: message.guild.id, allow: [PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.SendMessages] } ] });
...
const interval = setInterval(async () => { const vChan = await message.guild.channels.fetch(voice.id).catch(() => null); const tChan = await message.guild.channels.fetch(text.id).catch(() => null); if (!vChan || !tChan) return clearInterval(interval);
